...
class Transactions
{
    ...
    public static function install_tables(): void
    {
        ...
        dbDelta(self::build_table_sql($db, 'income', $charset));
        dbDelta(self::build_table_sql($db, 'expense', $charset));
        dbDelta(self::build_table_sql($db, 'purchase', $charset));
    }

    private static function build_table_sql(db $db, string $type, string $charset): string
    {
        $table = self::table_name($db, $type);
        $extra = '';
    ...
    public static function save(string $type, array $data)
    {
        ...
        if ($id > 0) {
            ...
            $updated = $db->update($table, $payload, ['id' => $id]);
    ...
    public static function delete(string $type, int $id): bool
    {
        ...
        $type = self::validate_type($type);
        $table = self::table_name($db, $type);
        return (bool) $db->delete($table, ['id' => $id]);
    }
    ...
    public static function get(string $type, int $id)
    {
        ...
        $type = self::validate_type($type);
        $table = self::table_name($db, $type);
        return $db->get_row($db->prepare("SELECT * FROM $table WHERE id = %d", $id));
    }
    ...
    public static function query(string $type, array $args = []): array
    {
        ...
        if (!empty($args['number'])) {
            $sql .= $db->prepare(' LIMIT %d', (int) $args['number']);
        }
    ...
    private static function validate_type(string $type): string
    {
        ...
        if (!array_key_exists($type, self::TYPES)) {
            die(esc_html__('Unknown transaction type.', 'arm-repair-estimates'));
        }
        return $type;
    }

    private static function table_name(db $db, string $type): string
    {
        return $db->prefix . self::TYPES[$type];
    }
    ...
    private static function sanitize_payload(string $type, array $data): array
    {
        $payload = [
            ...
            'description' => kses_post($data['description'] ?? ''),
        ];
